...
DatePaginator.prototype.createElement = function(date){
	...
	// прибавление нулей к дню и месяцу, если значение меньше 10, чтобы не происходило ошибок в iphone
	// для дня ("0" + this.getDate()).slice(-2)
	// для месяца ("0" + (this.getMonth() + 1)).slice(-2)
	a.setAttribute('data-date', date.getFullYear() + '-' + ("0" + (date.getMonth() + 1)).slice(-2) + '-' + ("0" + date.getDate()).slice(-2));
